...
addIcons(OiSidebarCollapse, OiSidebarExpand)
...
<template>
  ...
  <aside
    class="fixed left-0 top-0 hidden h-dvh flex-col border-r border-ink-border bg-surface lg:flex transition-[width] duration-300"
    :class="isCollapsed ? 'w-[72px]' : 'w-60'"
  >
    <!-- Header: brand + toggle, jadi satu baris -->
    <div
      class="flex items-center gap-3 px-5 py-5"
      :class="isCollapsed ? 'flex-col justify-center gap-2 px-2' : 'justify-between'"
    >
      <RouterLink
        :to="items[0]?.to ?? '/login'"
      ...
      <button
        ...
        class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg hover:bg-surface-hover"
        @click="toggleSidebar"
      >
         <OhVueIcon
          :name="isCollapsed ? 'oi-sidebar-expand' : 'oi-sidebar-collapse'"
          scale="1.1"
    ...
    <div class="border-t border-ink-border p-3">
      ...
      >
        ...
        />
        <span v-show="!isCollapsed">{{ settingsItem.label }}</span>
      </router-link>
